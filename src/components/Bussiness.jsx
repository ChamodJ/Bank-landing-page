...
const FeatureCard = ({icon , title, content, index}) => (
  <div className={`flex flex-row p-6 rounded-[20px] bg-dimBlue ${index !== features.length -1 ? 'mb-6' : 'mb-0'}` }>
    ...
    <div className={`flex flex-col flex-1 ml-4 text-start`}>
      ...
      <p className='font-poppins font-semibold text-white text-[18px] leading-[23.4px] mb-1'>{content}</p>
    </div>
...
const Bussiness = () => {
  return (
    <section id='features' className={`${layout.section}`}>
      <div className={`${layout.sectionInfo} text-start`}>
